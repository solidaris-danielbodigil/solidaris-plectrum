...
| Responsive reflow | `o-grid--auto-fit` / `o-grid--auto-fill` (+ optional `--sds-grid-min`) |
| Bespoke track list | `o-grid` + `style="--sds-grid-template-columns: …"` (Tier 2 escape hatch) |
...
| Need | Use |
|------|-----|
| Panel / section divider | `u-border-{side}` + `style="--sds-border-color: var(--sds-color-panel-border)"` |
| Component-token width | add `--sds-border-width: var(--…)` override on the same element |
...
Drawer TS: `SDS_DRAWER_APPEND_TO`, `SDS_DRAWER_CONTENT_STYLE`, `SDS_PANEL_BORDER_BOTTOM_STYLE`, `DrawerPosition`, `DetailListRow`, and `normalizeAccordionPanelIds()` live in `libs/ui/src/lib/drawer/`.
...
Standalone `libs/ui` components keep their own block (`c-affiliate-overview-card`, `c-list`). Shared layout primitives stay separate blocks (`c-detail-list`). Flat panel dividers use `u-border-bottom` + `--sds-border-color` in templates.

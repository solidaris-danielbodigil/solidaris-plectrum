...
**`doc-incapacite` mock (Figma comment — Scenario 1):** numbered stepper; steps **1 Paiement** then **2 Certificat** (not the primaire CIT → F.D.R. → Calcul trio). Paiement: _pas de paiement reçu pour le moment_; Certificat: prolongation maladie **25/12/2025–27/12/2025**.
...
| `doc-incapacite`           | demande primaire                        | 24/11/2025     | `En traitement`               | Step 1 Paiement                                       | Pas de paiement reçu; période 25–27/12/2025; prolongation CIT Accepté 25–27/12/2025             |
...
1. **`doc-incapacite`** (done — Figma Bodi Gil comment): numbered **Paiement** + **Certificat** prolongation (25–27/12/2025); extend later with attestation VA, carte de reprise if needed. _(Do not put attestation pédicure here — hors parcours.)_
...
2. **Optional:** split rechute into `doc-rechute-incapacite` (paiements + VA, numbered flux like `doc-incapacite`) as sibling list item. `doc-rechute` régime général mock is done.

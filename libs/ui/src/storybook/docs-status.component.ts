...
export { STATUS_PRESENTATION } from './governance';
export type { StatusSeverity, StatusPresentation } from './governance';

export const OWNER_LABEL: Readonly<Record<string, string>> = Object.fromEntries(registry.teams.map(team => [team.id, `${team.label} team`]));
const OWNER_IN_SENTENCE: Readonly<Record<string, string>> = Object.fromEntries(registry.teams.map(team => [team.id, `the ${team.label.toLowerCase() === 'design-system' ? 'design-system' : team.label} team`]));

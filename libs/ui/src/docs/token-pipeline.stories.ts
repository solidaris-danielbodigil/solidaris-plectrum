...
export const ChangeFlow: StoryObj = stepsStory([
  ...
  },
  {
    ...
    detail:
      'The PrimeUI theme generator commits to the staging branch design-tokens/sync. It has no access to main.',
  },
  {
    who: 'CI',
    tone: 'neutral',
    ...
    detail:
      'tokens-sync.yml rebuilds generated files from the staging dump, then compares Figma, the PrimeNG preset and the SCSS three ways and opens a pull request. Drift fails the check.',
  },
  {
    who: 'Developer',
    tone: 'system',
    ...
    detail:
      'tokens.json is updated. Generated SCSS and the Storybook manifest are rebuilt. Manual overrides stay in extend.ts.',
  },
  {
    who: 'CI',
    tone: 'neutral',
    title: 'Storybook reflects the change',
    detail:
...
export const Rules: StoryObj = cardsStory([
  {
    title: 'CSS is the API',
    lead: 'Applications and libs/ui use var(--pds-*) and o- / c- / u- classes. PrimeNG theme helpers ($dt, dt, usePreset, updatePreset) are not imported at runtime.',
  },
  {
    title: 'Hybrid colours',
    lead: 'A mapped colour is emitted as var(--p-primary-600, #487395): the live PrimeNG value with the Figma literal as fallback. Both values are generated from data.',
  },
  {
    title: '--pds- prefix',
    lead: 'All declarations are prefixed. Bare --spacing-* / --text-* / --font-* remain for one release as aliases in _settings.legacy-aliases.scss. BEM class names such as o-layout--gap-2 are unchanged.',
  },
  {
  ...
  },
  {
    ...
    lead: 'The Foundations Figma file exposes them as text styles, not variables. They are authored in SCSS and proposed back to Figma when required.',
  },
]);

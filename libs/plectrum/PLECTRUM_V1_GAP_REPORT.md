...
**Date:** 2026-06-17  
**Scope:** `Plectrum_v1/ts` vs `Plectrum_v0.6` vs `tokens.json` vs PDS primitives  
...
| **PDS** | Gap or fix in `libs/styles` (primitives, `01-settings` bridges, `06-components` BEM) |
| **Both** | Preset default and PDS bridge/wiring both contribute |
...
| `extend` / `emutnav` / PDS hardcoded primitives | Low — parallel SSOT | **PDS** |
| PDS `primary-600` / `surface-50` drift | P8 — separate initiative | **PDS** |
...
| **extend** | `emutnav`, `transparant.*`, `surface.75`, `focus.color`, `primary.75` | `basic.solidaris`, `plectrum.*`, `functional.link` | v1 preset does not emit v0.6 extend vars; PDS SCSS uses hardcoded `--PDS-*` instead | **PDS** |
...
| **index** | Includes `steps`, `tabmenu`, `tabview`, `inputchips`, `colorpicker`, `inlinemessage` | Includes `tabs` (unified); drops v0.6-only components | Low — unused in PDS apps | **Preset** |
...
**Source:** **Both** (preset `borderWidth` + PDS `c-accordion--chromeless` token-bridge wiring)
...
| Role | tokens.json (Figma) | v0.6 | v1 | PDS primitive |
|------|---------------------|------|-----|---------------|
| Panel / drawer border | `#e7e7e7` (drawer border) | `surface.200` / transparent variants | `surface.200` in overlays | `--PDS-color-surface-border-drawer` **#e7e7e7** ✓ |
| Primary 600 | `{primary.600}` semantic | Custom scale in primitive | `#487395` scale | `--PDS-color-primary-600` **#3f5870** (drift) |
| Surface 50 (page) | neutral.50 / gray | `#f9f9f9` | `{neutral.50}` → `#fafafa` | `--PDS-color-surface-50` **#f6f6f6** (drift) |
| Overlay shadow | drawer shadow tokens | `{overlay.modal.shadow}` | Hardcoded `0 8px 10px…` | `--PDS-shadow-*` separate |
| surface.0 | `{extremes.white}` | `#ffffff` | `{extremes.white}` **undefined** | `--PDS-color-surface-0` **#ffffff** ✓ |

**Note:** PDS primitives were authored against v0.6 + Figma audits; v1 Figma export uses a different primary/surface story. PDS overrides (`--p-*` in `01-settings`) bridge many gaps.

---

## 4. PDS primitives vs Figma drift

Tokens apps actually use:

| PDS variable | Value | Figma / v1 | Status |
|--------------|-------|------------|--------|
| `--PDS-color-panel-border` | `#e7e7e7` | drawer border | ✓ Aligned |
| `--PDS-color-surface-50` | `#f6f6f6` | neutral.50 `#fafafa` | Minor drift |
| `--PDS-color-primary-600` | `#3f5870` | v1 `#487395` | Intentional until PDS refresh |
| `--PDS-color-emutnav-*` | rose/black-alpha | v0.6 `extend.emutnav` only | PDS hardcoded — **no v1 preset dependency** |
| `--PDS-color-surface-75` | `#ededed` | v0.6 `extend.surface.75` | PDS only — v1 components don't reference |
...
| P6 | extend merge (v0.6) | **Skip** — no unresolved v1 refs to `emutnav`/`transparant`/`surface.75` | **PDS** |
| P7 | Dark colorScheme | **Out of scope** — no full dark mode | **Out of scope** |
| P8 | PDS primary refresh | Separate initiative — do not mirror `tokens.json` | **PDS** |
...
- Regenerating PDS primitives from Figma

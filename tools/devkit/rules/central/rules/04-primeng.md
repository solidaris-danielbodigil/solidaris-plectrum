...
- Exclusive option sets (`p-selectButton` for **at most 5** options; more than 5 → `p-select` or `p-autocomplete`)
...
.c-my-feature__nav-button {
  ...
  --p-button-outlined-secondary-hover-background: var(
...
- The figure is an Angular component in `libs/ui/src/storybook/` (`pds-docs-steps`, `pds-docs-cards`, `pds-docs-callout`, `pds-docs-status`, `pds-docs-link`, `pds-docs-anatomy`, `pds-docs-sync-checks`, `pds-docs-sync-changes` — the last two render the generated sync record `sync-report.generated.ts` with `p-tag`, `p-message` and `p-table`). `pds-docs-anatomy` overlays `p-badge` callouts and SVG leader lines on a live specimen; positions are measured from the CSSOM. Missing parts are a `p-message`, not numbered callouts. Generated records get the same treatment: `pds-docs-changesets` / `pds-docs-releases` render `changelog.generated.ts` (What's new) with `p-card`, `p-timeline` and `p-tag`, and `pds-docs-catalogue` renders Find a component with `p-table`, `p-select` and `p-tag`. Empty records show `pds-empty-state`, never a bare sentence.
...
- Two docs figures are custom because PrimeNG has no equivalent: the SVG `<Diagram>` in
  `libs/ui/.storybook/docs-figures.ts` (boxes-and-arrows) and `pds-docs-hero`
  (`libs/ui/src/storybook/docs-hero.component.ts`) — the Introduction banner built from the Figma
  thumbnail (node 9967:29548): exported plectrum shape from `libs/assets`, Solidaris logo pill,
  Agenda title, with `pButton` link anchors for the calls to action. Its tokens live in
  `01-settings/_settings.docs-hero.scss`, its BEM in `06-components/_components.docs-hero.scss`.
  Document any further exception the same way.
...
h(
  'ol',
  { className: 'c-docs-steps' },
  steps.map((s, i) =>
    h('li', null, h('span', { className: 'badge' }, i + 1), s.title),
  ),
);
```

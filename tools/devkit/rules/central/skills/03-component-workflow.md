...
```bash
npm run pds:component -- --name=example-card --owner=design-system   # core team → governance.status 'core'
npm run pds:component -- --name=team-card --owner=ishare          # application team → 'candidate', title Patterns/{App}/…
```

This creates the component folder (no colocated stylesheet), the metadata contract with its `governance` block, the `_components.{name}.scss` partial with its core-barrel `@forward`, and the Storybook story template — then regenerates the metadata registry, central index and eligible package exports. Core SCSS joins the main stylesheet; candidate SCSS stays in the Storybook-only candidate stylesheet.
...
- [ ] `.metadata.ts` has an immutable `component.id`, registered owner and governance-compatible `distribution`; `contracts:check` passes with no metadata TODOs

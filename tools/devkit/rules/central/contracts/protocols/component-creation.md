...
| Application team registered in `registry.json` | `candidate`    | `Patterns/{App}/…`                 | Core team moves it: generic API, tokens to shared settings, title to Custom components / Shell, `status: 'core'`, `owner: 'design-system'` (docs/component-promotion.md) |
...
(`libs/ui/src/docs/docs-figure-stories.ts`) as the first figure under the `h1`; CSS-only blocks use colocated metadata too.
...
The generator creates the current contract from `schema/component.schema.ts`. Its inferred TypeScript type and generated JSON Schema share that source. Keep `component.id` unchanged on rename or promotion. Set governance and distribution together: core Angular `entryPoint: '.'`, candidate `kind: 'local'`, app pattern `./patterns/<owner>` or local. A replacement gets a new ID and the old record's `replacementId`.

Run `npm run contracts:generate` after metadata changes. It generates the registry, index and package exports. Complete every metadata TODO before `contracts:check`. Never edit the generated barrels.
...
- [ ] Component and public types exported from its own `index.ts`; `distribution` declares the package entry; generated exports pass `contracts:generate -- --check`

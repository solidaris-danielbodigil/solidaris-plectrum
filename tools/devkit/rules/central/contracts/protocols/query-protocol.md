...
1. **Read `.ai/contracts/index.json`** (entries and relationships are keyed by immutable `component.id`, e.g. `plectrum:form-field`; display names are fields) — workspace map, component inventory, relationships (offline). When `npm run storybook` is up, Storybook MCP at `http://localhost:6006/mcp` is the live catalogue next to the index.
...
8. Fill the scaffold metadata TODOs and component states; figures and package/catalogue registration are already generated. Run contracts:generate, contracts:check and docs:check.
```
